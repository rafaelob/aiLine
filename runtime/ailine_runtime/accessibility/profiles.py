...
class AccessibilityNeeds(BaseModel):
    ...
    autism: bool = Field(False, description="TEA / previsibilidade, comunicação clara e sensorial.")
    adhd: bool = Field(False, description="TDAH / atenção, organização, tempo e autorregulação.")
    learning: bool = Field(False, description="Dificuldades de aprendizagem (ex.: dislexia/defasagem).")
    hearing: bool = Field(
        False,
...
class UiPreferences(BaseModel):
    ...
    large_print: bool = Field(False, description="Fonte maior + espaçamento.")
    high_contrast: bool = Field(False, description="Contraste reforçado.")
    dyslexia_friendly: bool = Field(False, description="Ajustes de tipografia/espacamento para leitura.")
    reduce_motion: bool = Field(True, description="Respeitar prefers-reduced-motion e evitar transições chamativas.")


class AutismSupportSettings(BaseModel):
    intensity: SupportIntensity = Field("medium", description="Intensidade de suportes de previsibilidade e sensorial.")
    sensory_sensitivity: SupportIntensity = Field("medium", description="Sensibilidade sensorial (luz/som/ruído).")
    require_visual_schedule: bool = Field(True, description="Gerar agenda/rotina e cronograma visual da aula.")
    require_transition_scripts: bool = Field(True, description="Incluir scripts de transição (ex.: 'Agora vamos...').")
    ...
    avoid_figurative_language: bool = Field(True, description="Evitar metáforas/ambiguidade em instruções críticas.")


class ADHDSupportSettings(BaseModel):
    intensity: SupportIntensity = Field("medium", description="Intensidade de suportes de atenção/organização.")
    focus_window_minutes: int = Field(8, ge=3, le=20, description="Janela de foco típica para chunking (min).")
    ...
    require_checkpoints: bool = Field(True, description="Incluir checkpoints do tipo 'feito' e checagem rápida.")
    require_timer_prompts: bool = Field(True, description="Incluir prompts de timer/tempo restante por etapa.")
...
class LearningSupportSettings(BaseModel):
    ...
    target_reading_level: Literal["simple", "standard"] = Field("simple", description="Nível alvo da versão aluno.")
    require_examples_first: bool = Field(True, description="Exemplo antes de abstração quando possível.")
    require_glossary: bool = Field(True, description="Gerar glossário de termos difíceis (curto).")
...
class HearingSupportSettings(BaseModel):
    intensity: SupportIntensity = Field("medium", description="Intensidade de suportes auditivos.")
    require_captions: bool = Field(True, description="Vídeo/áudio exige legendas.")
    require_transcript: bool = Field(True, description="Áudio exige transcrição.")
...
class VisualSupportSettings(BaseModel):
    intensity: SupportIntensity = Field("medium", description="Intensidade de suportes visuais.")
    require_alt_text: bool = Field(True, description="Imagens/figuras exigem texto alternativo.")
    ...
    require_large_print: bool = Field(True, description="Gerar variante large print.")
    braille_ready: bool = Field(False, description="Se true: requer revisão humana e pipeline para BRF/Braille-ready.")


class SupportSettings(BaseModel):
    autism: AutismSupportSettings = Field(default_factory=AutismSupportSettings)
    adhd: ADHDSupportSettings = Field(default_factory=ADHDSupportSettings)
    learning: LearningSupportSettings = Field(default_factory=LearningSupportSettings)
    hearing: HearingSupportSettings = Field(default_factory=HearingSupportSettings)
    visual: VisualSupportSettings = Field(default_factory=VisualSupportSettings)
...
class ClassAccessibilityProfile(BaseModel):
    ...
    needs: AccessibilityNeeds = Field(default_factory=AccessibilityNeeds)
    ui_prefs: UiPreferences = Field(default_factory=UiPreferences)

    # Ajustes finos (opcionais no MVP; úteis para "englobar tudo")
    supports: SupportSettings = Field(default_factory=SupportSettings)

    notes: str | None = Field(None, description="Observações curtas (evitar PII).")
...
def human_review_flags(class_profile: ClassAccessibilityProfile | None) -> tuple[bool, list[str]]:
    ...
    if n.hearing and s.hearing.sign_language in ("libras", "asl", "other"):
        reasons.append(
            "Preferencia por lingua de sinais indicada"
            " -- exige revisao humana (interprete/material bilingue)."
        )
