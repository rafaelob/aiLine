...
12. **`create_react_agent` from `langgraph.prebuilt` is deprecated** -- LangGraph v1 moved these prebuilts to `langchain.agents`. Use `from langchain.agents import create_agent(model, tools, system_prompt=...)`. `langgraph-supervisor` / `langgraph-swarm` packages may still use it internally; check the package version before switching.
